...
const GoalSettingPage: React.FC = () => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      const response = await axios.post("http://localhost:3000/goal", payload);
  ...
  return (
    <PageWrapper>
      <Form onSubmit={handleSubmit}>
        <Title>운동 목표를 설정해주세요</Title>
        <SubText>본인의 주요 운동 목표를 선택하세요.</SubText>
