...
const levelColor: Record<string, string> = {
  쉬움: "#FEF3C7", 보통: "#FFE8B3", 어려움: "#FFD6A5",
};

const textColor: Record<string, string> = {
  쉬움: "#CA8A04", 보통: "#D97706", 어려움: "#EA580C",
};
...
const RoutinePreviewPage: React.FC = () => {
 ...
 useEffect(() => {
  ...
  const fetchAndGenerateRoutine = async () => {
    try {
      // 1. 루틴 재생성 요청
      await axios.post("http://localhost:3000/routine", {
        userId,
      });
  ...
  return (
    <Container>
      <HeaderWrapper>
        ...
        <HeaderButton onClick={() => navigate("/mypage")}>마이페이지</HeaderButton>
      </HeaderWrapper>
                ...
                >
                  {routine[day].map((item, idx) => {
                    ...
                    return (
                      ...
                      >
                        <div>
                          <p style={{ fontWeight: 500, color: "#374151" }}>{item.part}</p>
