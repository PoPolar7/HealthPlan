...
const BodyInfoEditPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (email) {
      axios
        .get(`http://localhost:3000/body-info/${encodeURIComponent(email)}`)
        .then((res) => {
          setHeight(String(res.data.height));
          setWeight(String(res.data.weight));
          setFat(String(res.data.bodyFat));
          setMuscle(String(res.data.muscleMass));
        })
        .catch(() => {
          console.warn("신체 정보 조회 실패");
        });
    }
  }, []);
...
export default BodyInfoEditPage;
