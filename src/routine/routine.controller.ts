...
async getExerciseInfo(
  ...
  @Query('userId') userId?: string, // ✅ 사용자 ID 추가
...
async getExerciseList(
  ...
  @Query('userId') userId?: string // ✅ userId 쿼리 파라미터 받기
) {
  const dbPath = path.join(__dirname, '../../data/routines.json');
  if (!fs.existsSync(dbPath)) {
    throw new NotFoundException('routines.json 파일이 없습니다.');
  ...
  // ✅ 여기서 선언해야 TypeScript 오류 없음
  let completedNames: string[] = [];

  if (userId) {
    const completedDocs = await this.completedModel.find({ userId }).select('name');
    completedNames = completedDocs.map((doc) => doc.name);
  }

  // ✅ 여기서 completedNames 사용 가능
